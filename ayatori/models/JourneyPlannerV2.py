...
class JourneyLeg:
    ...
    def __repr__(self):
        if self.leg_type == "walk":
            ...
            mins = self.duration.total_seconds() / 60
            return (
                f"Transit(Route {self.route_id}, {self.from_stop}->{self.to_stop}, {mins:.1f}min)"
            )
...
class Journey:
    ...
    def __init__(self, origin_coords: tuple[float, float], destination_coords: tuple[float, float]):
        """
        Crea un nuevo viaje.
    ...
    def get_departure_time(self) -> datetime | None:
        """Hora de salida del viaje"""
        return self.legs[0].start_time if self.legs else None

    def get_arrival_time(self) -> datetime | None:
    ...
    def __repr__(self):
        ...
        return (
            f"Journey({dep}->{arr}, {duration_mins:.0f}min, "
            f"{self.number_of_transfers} transfers, {self.total_walking_distance:.2f}km walk)"
        )


class JourneyPlannerV2:
    """Planificador multimodal de alto nivel sobre ``ConnectionScanAlgorithm``.

    Wrapper orientado a casos de uso comunes (planificar un viaje, listar
    paradas cercanas a un origen). El motor real es CSA; esta clase ofrece
    una API más cercana a lenguaje natural y maneja el caso "ningún viaje
    encontrado" reintentando con un radio de caminata mayor antes de rendirse.

    Attributes:
        gtfs: instancia de :class:`GTFSData`.
        max_walking_distance: km máximos hasta la primera/última parada.
        walking_speed: km/h, default 5.0.

    Notes:
        Para control fino sobre transbordos, costo generalizado o presupuesto
        total de caminata, usar :class:`ConnectionScanAlgorithm` directamente
        con un ``CSAConfig`` custom.
    """

    def __init__(self, gtfs_data, max_walking_km: float = 1.0, walking_speed_kmh: float = 5.0):
        """
    ...
    def find_nearby_origin_stops(
        self, origin_coords: tuple[float, float], max_stops: int = 5
        ...
        nearby = self.gtfs.get_nearby_stops(
            origin_coords, margin_km=self.max_walking_distance, max_stops=max_stops
        )
    ...
    def _convert_csa_journey_to_legacy(self, csa_journey) -> Journey | None:
        """
    ...
    def _plan_journey_simple(
        ...
        initial_walk = JourneyLeg(
            leg_type="walk", start_time=walk_start, end_time=walk_end, distance=origin_distance
        )
        ...
        transit_duration = self._estimate_transit_time(
            best_route, best_origin_stop, best_destination_stop, transit_start
        )
        ...
        dest_distance = next(
            (d for s, d, _ in destination_stops if s == best_destination_stop), 0.5
        )
    ...
    def _estimate_transit_time(
        self, route_id: str, from_stop: str, to_stop: str, departure_time: datetime
    ) -> timedelta:
        """
        ...
        try:
            ...
            if hasattr(self.gtfs, "get_arrival_times"):
                ...
                arrival_info = self.gtfs.get_arrival_times(route_id, from_stop, departure_date_str)
...
def create_journey_planner_v2(
    gtfs_data, max_walking_km: float = 1.0, walking_speed_kmh: float = 5.0
) -> JourneyPlannerV2:
    """

#!/usr/bin/env python
"""
Journey Planner — DEPRECATED.

Implementación V1, conservada para retro-compatibilidad. Será removida en 0.3.0.
Usar ``ayatori.models.JourneyPlannerV2`` o ``ConnectionScanAlgorithm``.
"""
...
_DEPRECATION_MSG = (
    "JourneyPlanner (V1) is deprecated and will be removed in 0.2.x/0.3.0. "
    "Use JourneyPlannerV2 (create_journey_planner_v2) or ConnectionScanAlgorithm directly."
)
...
class JourneyLeg:
    ...
    def __repr__(self):
        if self.leg_type == "walk":
            ...
            return (
                f"Transfer({self.transfer_from}->{self.transfer_to}, {self.duration / 60:.1f}min)"
            )
...
class JourneyPlanner:
    ...
    def __init__(
        self, gtfs_data, max_walking_distance_km: float = 1.0, walking_speed_kmh: float = 5.0
    ):
        """
    ...
    def find_nearby_destination_stops(
        self, destination_coords: tuple[float, float], max_stops: int = 5
    ) -> list[tuple[str, float, float]]:
        """
...
# Función de conveniencia — DEPRECATED
def create_journey_planner(
    gtfs_data, max_walking_km: float = 1.0, walking_speed_kmh: float = 5.0
) -> JourneyPlanner:
    """DEPRECATED. Usar ``create_journey_planner_v2``."""
    warnings.warn(_DEPRECATION_MSG, DeprecationWarning, stacklevel=2)
